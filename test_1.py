...
zoom_ax = plt.axes([0.18, 0.14, 0.04, 1])
zoom_slider = Slider(zoom_ax, label='Zoom', valmin=1e8, valmax=6e12, valinit=6e12)
def update_zoom(val):
...
@njit
def velocity_verlet(pos, vel, steps, dt, M_central):
    xs = np.zeros(steps)
    ys = np.zeros(steps)
    ...
    for i in range(steps):
        ...
        acc = -G * M_central * pos / r**3

        # Velocity Verlet update
        pos_next = pos + vel * dt + 0.5 * acc * dt**2
        r_next = np.sqrt(pos_next[0]**2 + pos_next[1]**2)
        acc_next = -G * M_central * pos_next / r_next**3
        vel = vel + 0.5 * (acc + acc_next) * dt
        pos = pos_next
...
@njit
def generate_orbit(
    eth_distance, eth_velocity, eth_mass,
    mer_distance, mer_velocity,
    ven_distance, ven_velocity,
    mars_distance, mars_velocity,
    jup_distance, jup_velocity, jup_mass,
    plu_distance, plu_velocity,
    sat_distance, sat_velocity, sat_mass,
    uran_distance, uran_velocity, uran_mass,
    nep_distance, nep_velocity, nep_mass,
    steps, dt
):
    # Initial conditions for all planets (pos along x-axis, vel along y-axis)
    ...
    # Orbits
    earth_x, earth_y = velocity_verlet(earth_pos, earth_vel, steps, dt, M_sun)
    mercury_x, mercury_y = velocity_verlet(mercury_pos, mercury_vel, steps, dt, M_sun)
    venus_x, venus_y = velocity_verlet(venus_pos, venus_vel, steps, dt, M_sun)
    mars_x, mars_y = velocity_verlet(mars_pos, mars_vel, steps, dt, M_sun)
    jupiter_x, jupiter_y = velocity_verlet(jupiter_pos, jupiter_vel, steps, dt, M_sun)
    saturn_x, saturn_y = velocity_verlet(saturn_pos, saturn_vel, steps, dt, M_sun)
    uranus_x, uranus_y = velocity_verlet(uranus_pos, uranus_vel, steps, dt, M_sun)
    neptune_x, neptune_y = velocity_verlet(neptune_pos, neptune_vel, steps, dt, M_sun)
    pluto_x, pluto_y = velocity_verlet(pluto_pos, pluto_vel, steps, dt, M_sun)
